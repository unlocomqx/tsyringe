...
class DependencyContainer {
    ...
    register(token, provider, options = { singleton: false }) {
        ...
        if (providers_1.isTokenProvider(provider) && typeof provider.useToken === "string") {
            exports.lazyRegistrationInfo.set(provider.useToken, token);
        }
    ...
    assignLazyProps(token) {
        ...
        const lazyRegistration = exports.lazyRegistrationInfo.get(key) || [];
        if (lazyRegistration) {
            exports.lazyPropsInfo.delete(lazyRegistration);
        }
        propsInfos.map((propsInfo) => {
            propsInfo.target[propsInfo.property] = this.resolve(token);
        });
    }
    assignLazyRegistration(token) {
        const key = token.name;
        const lazyRegistration = exports.lazyRegistrationInfo.get(key) || [];
        const propsInfos = exports.lazyPropsInfo.get(lazyRegistration) || [];
        if (propsInfos.length) {
            exports.lazyPropsInfo.set(key, propsInfos);
        }
    }
    isRegistered(token) {
...
exports.lazyRegistrationInfo = new Map();
